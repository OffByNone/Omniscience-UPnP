...
var SOAPService = (function () {
    function SOAPService(fetch, domParser, stringUtilities) {
    ...
    _createClass(SOAPService, [{
        ...
        value: function post(url, serviceName, methodName, parameters) {
            ...
            return this._fetch(url, {
                ...
                var responseXML = _this._DOMParser.parseFromString(responseText, 'text/xml'); //todo: what if this fails [hint: it does... all the time]
        ...
        value: function parametersToXml(parameters) {
            var xml = '';
            for (var parameterKey in parameters) if (parameters.hasOwnProperty(parameterKey)) xml += '<' + parameterKey + '>' + parameters[parameterKey] + '</' + parameterKey + '>';
            return xml;
        }
    }]);
