module.exports = {
	...
	callService: (serviceControlUrl, serviceUUID, serviceMethod, data) => {
        ...
        var serviceClass = this.executableServices[serviceUUID];
    ...
    addExecutableService: (executableService, serviceUUID) => {
		if (!executableService) throw new Error("Argument 'executableService' cannot be null.");
		if (!serviceUUID) throw new Error("Argument 'serviceUUID' cannot be null.");

		if (!this.executableServices.hasOwnProperty(serviceUUID))
			this.executableServices[serviceUUID] = executableService;
    }
};