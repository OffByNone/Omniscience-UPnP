...
class DeviceService extends Eventable {
	constructor(deviceFactory, deviceLocator, storageService, notifications, fetch, md5, timers) {
		super();
	...
	stop() {
		...
		this._timers.clearInterval(this._searchIntervalId);
	}
	search() {
		if (!this._isInitialized) {
			...
			this._searchIntervalId = this._timers.setInterval(() => this._deviceLocator.search(this.devices), Constants.searchIntervalInSeconds * 1000);
		}
		else
			this._deviceLocator.search(this.devices);
	}
	...
	_copyObj(copyTo, copyFrom) {
		for (let prop in copyFrom) {
			if (copyFrom.hasOwnProperty(prop)) {
				if (typeof copyFrom[prop] === "object" && copyFrom[prop] !== null && copyFrom.toString() === "[object Object]") //second part is to stop it form trying to copy urls as objects in firefox which wont work.
