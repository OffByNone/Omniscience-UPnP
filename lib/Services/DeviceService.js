...
class DeviceService extends Eventable {
	...
	loadDevices() {
		return new Promise((resolve, reject) => {
			if (!this.devices) {
				this._storageService.get("devices").then((devices) => {
					this.devices = Array.isArray(devices) ? devices : [];
					this.devices.forEach(device => this.emit("deviceBuilt", device));
					resolve();
				});
			}
			else {
	...
	search() {
		if (!this._isInitialized) {
			...
			this.loadDevices().then(() => this._deviceLocator.search(this.devices));
		}
		else
	...
	_copyObj(copyTo, copyFrom) {
		for (let prop in copyFrom) {
			if (copyFrom.hasOwnProperty(prop)) {
				if (typeof copyFrom[prop] === "object" && copyFrom[prop] !== null && !copyFrom[prop].hasOwnProperty("scheme")) //second part is ugly hack to stop it form trying to copy urls in firefox which wont work.
