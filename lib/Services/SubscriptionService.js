...
class SubscriptionService {
	...
	subscribe(directResponsesTo, subscriptionUrl, timeoutInSeconds, subscriptionId) {
		...
		return this._fetch(subscriptionUrl, {
			...
			if (!response.ok) {
				...
				if (response.status == Constants.PreconditionFailed) {
					...
					return this.subscribe(directResponsesTo, subscriptionUrl, timeoutInSeconds, subscriptionId);
				}
				else
