...
class DeviceFactory {
    constructor(xmlParser, urlProvider, md5, upnpServiceFactory, upnpExtensionInfoFactory, xhr, btoa) {
    ...
    create(device, responseText, location, fromAddress, serverIP) {
		...
		return this._parseDeviceIcons(device, deviceXml, location, base).then(() => {
		...
		return new Promise((resolve, reject) => {
			...
			iconsXml.forEach((iconXml) => {
				...
				if (icon.url && icon.url.href) {
					this._getImage(icon.url.href, icon.mimeType).then(function (response) {
		...
		return new Promise((resolve, reject) => {
			...
			xhr.onload = (e) => {
				if (e.target.status == 200) {
					...
					let base64 = this._btoa(binaryString.join(''));
