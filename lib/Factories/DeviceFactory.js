...
class DeviceFactory extends Eventable {
	...
	_parseDeviceAttributes(deviceXml, responseHash, serverIP, fromAddress, location, upnpVersion, base) {
		...
		device.upnpVersion = upnpVersion;
		...
		let subDevices = this._xmlParser.getElements(deviceXml, ":scope > deviceList > device");
		...
		servicesXml.forEach((serviceXml) => {
			servicePromises.push(this._upnpServiceFactory.create(serviceXml, location, base, serverIP).then((serviceInfo) => {
				serviceInfo.fontIcons.forEach((fontIcon) => device.fontIcons[fontIcon] = fontIcon);
				device.services.push(serviceInfo);
				return true;
			}));
		});
		//todo: what if one service fails?  do we really want to not show the device because a single service on it failed?
		Promise.all(servicePromises).then(() => {
			this._parseDeviceIcons(device, deviceXml, location, base).then(() => this.emit("deviceBuilt", device));
		});
	}
	_parseDeviceIcons(device, deviceXml, location, base) {
		return new Promise((resolve, reject) => {
			...
			if (device.icon && device.icon.url && device.icon.url.href) {
				this._getImage(device.icon.url.href, device.icon.mimeType).then(function (response) {
					device.icon.base64Image = response;
					resolve();
				});
			} else {
				device.icon = new Icon();
				device.icon.url = Constants.IconDefaults[device.type.raw];
				device.icon.base64Image = Constants.IconDefaults[device.type.raw];
				device.icon.mimetype = "image/png";
				device.icon.width = 1;
				device.icon.height = 1;
				device.icon.depth = 1;
				resolve();
			}
		});
