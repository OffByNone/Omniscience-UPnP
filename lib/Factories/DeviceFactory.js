...
class DeviceFactory {
    constructor(xmlParser, urlProvider, md5, serviceInfoFactory) {
        this._xmlParser = xmlParser;
        this._urlProvider = urlProvider;
        this._md5 = md5;
		this._serviceInfoFactory = serviceInfoFactory;
    }
    create(device, responseXml, location, fromAddress, serverIP) {
		...
		this._parseDeviceXml(device, deviceXml, base, location, serverIP);
		var servicesXml = this._xmlParser.getElements(deviceXml, "serviceList service");
		
		servicesXml.forEach((serviceXml) => {
			var serviceInfo = this._serviceInfoFactory.create(serviceXml, location, base, serverIP);
		...
		device.responseHash = this._md5(responseXml);
		device.fromAddress = fromAddress;
		device.serverIP = serverIP;
    }
	_parseDeviceXml(device, deviceXml, location, base) {
