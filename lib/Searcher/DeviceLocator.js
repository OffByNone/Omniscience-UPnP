...
class DeviceLocator extends Eventable {
	constructor(timer, fetch, activeSearcher, passiveSearcher, xmlParser) {
		super();
		this._timer = timer;
		this._fetch = fetch;
	...
	search(devices) {
		...
		devices.forEach((device) => {
			this._checkForLostDevice(device.ssdpDescription, device.id, false).then(found => {
				if (!found) this.emit("deviceLost", device.id);
			});
		});
	...
	_deviceFound(headers, ignoreDebounce) {
		...
		this._deviceTimeouts[id] = this._timer.setTimeout(() => {
			this._checkForLostDevice(headers.location, id, true).then(found => {
				if (!found)
	...
	_checkForLostDevice(location, id) {
		return this._fetch(location).then(
			(response) => {
				if (!response.ok)
					...
					let deviceIdElements = this._xmlParser.getElements(responseXml, "UDN");
					return deviceIdElements.some(deviceIdElement => id === deviceIdElement.innerHTML.replace("uuid:", ""));
			...
			(response) => { return false; });/*error occured while trying to ping device, consider lost.*/
	}
}
